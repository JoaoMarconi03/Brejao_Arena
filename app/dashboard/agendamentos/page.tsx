...
const SLOT_H = 72 // px por slot de 30 min
const HOUR_START = 8
const HOUR_END = 23
...
const slots = Array.from({ length: (HOUR_END - HOUR_START) * 2 }, (_, i) => {
  ...
  const m = totalMin % 60
  return { label: `${h.toString().padStart(2, "0")}:${m.toString().padStart(2, "0")}`, h, m, index: i }
})
...
function toTopPx(h: number, m: number) {
  return ((h - HOUR_START) * 2 + m / 30) * SLOT_H
}

function toDurationPx(durationMin: number) {
  return (durationMin / 30) * SLOT_H
}
...
/** Um slot de 30 min está ocupado se algum agendamento (não cancelado) o cobre */
...
/** Retorna só os slots onde inicio+duração cabe sem conflito e sem ultrapassar 23h */
function getAvailableSlots(duracaoMin: number, ags: Agendamento[]) {
  return slots.filter((slot) => {
    ...
    // não pode ultrapassar o horário de encerramento
...
export default function AgendamentosPage() {
  ...
  function openDialog(slot?: { h: number; m: number }) {
    ...
    setForm({ inicio, cliente: "", duracao, tipo: "AVULSO", valor: "60", observacao: "" })
    setDialogOpen(true)
  }

  function handleSlotClick(slot: (typeof slots)[0]) {
    if (isSlotOccupied(slot, agendamentos)) return
    openDialog(slot)
  }
  ...
  function handleTipoChange(val: string) {
    setForm((f) => ({ ...f, tipo: val, valor: val === "MENSALISTA" ? "0" : f.valor }))
  }
          ...
          {Array.from({ length: 7 }, (_, i) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-base font-bold leading-tight ${today && !active ? "text-primary" : ""}`}>
                  {format(d, "d")}
                </span>
              </button>
            )
            ...
            {slots
              ...
              .map((s) => (
                <div
                  ...
                  className="flex items-start pt-2 pr-3 justify-end"
                >
              ...
              return (
                <div
                  ...
                  className={`border-b transition-colors relative ${
                    slot.m === 0 ? "border-border" : "border-border/30"
                  } ${
                    occupied
                ...
                >
                  {/* Ícone de cadeado no slot bloqueado */}
                  {occupied && (
                    <Lock className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground/40" />
                  )}
                </div>
              )
            })}

            {/* Booking cards (sobrepostos) */}
            {agendamentos.map((ag) => {
              const top = toTopPx(ag.inicio.h, ag.inicio.m)
              const height = toDurationPx(ag.duracaoMin)
              const endTotalMin = toMin(ag.inicio.h, ag.inicio.m) + ag.duracaoMin
              const endH = Math.floor(endTotalMin / 60)
              const endM = endTotalMin % 60
              const inicioStr = `${ag.inicio.h.toString().padStart(2, "0")}:${ag.inicio.m.toString().padStart(2, "0")}`
              const fimStr   = `${endH.toString().padStart(2, "0")}:${endM.toString().padStart(2, "0")}`
              const isCompact = height < 100
              return (
                <div
                  key={ag.id}
                  style={{ top, height, left: 4, right: 4 }}
                  className={`absolute rounded-lg border overflow-hidden cursor-pointer ${statusColor[ag.status]}`}
                >
                  {/* Faixa de horário — sempre no topo, grande e legível */}
                  <div className="bg-black/20 px-3 py-1 flex items-center justify-between gap-2">
                    <span className="font-mono font-bold text-base leading-none tracking-wide">
                      {inicioStr}
                      <span className="opacity-60 mx-1.5 font-normal">→</span>
                      {fimStr}
                    </span>
                    <Badge className="text-[10px] px-1.5 py-0 h-4 bg-black/20 border-0 text-inherit shrink-0">
                      {ag.tipo === "MENSALISTA" ? "Mensal" : "Avulso"}
                    </Badge>
                  </div>
                  ...
                  {!isCompact && (
                    <div className="px-3 py-1.5 flex items-center justify-between gap-2">
                      ...
                      {ag.valor > 0 && (
                        <span className="text-xs opacity-80 shrink-0">R$ {ag.valor}</span>
                      )}
                    </div>
                  )}
                </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-card border-border sm:max-w-md">
          ...
          <div className="space-y-4 pt-1">
            {/* Aviso: nenhum horário disponível para essa duração */}
            {availableSlots.length === 0 && (
              <div className="flex items-start gap-2 px-3 py-3 rounded-lg bg-destructive/15 border border-destructive/30 text-destructive text-sm">
                ...
                <span>
                  Não há horários disponíveis para <strong>{form.duracao === "60" ? "1 hora" : form.duracao + " min"}</strong> hoje.
                  Tente uma duração menor ou escolha outro dia.
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  value={inicioValido ? form.inicio : ""}
                  onValueChange={(v) => setForm((f) => ({ ...f, inicio: v }))}
                  disabled={availableSlots.length === 0}
                >
                  <SelectTrigger className="bg-secondary border-border text-foreground">
                    <SelectValue placeholder="Nenhum disponível" />
                  </SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={form.duracao} onValueChange={handleDurationChange}>
                ...
                <SelectContent className="bg-popover border-border">
                  {[
                    { v: "60",  l: "1 hora" },
                    { v: "90",  l: "1 hora e 30 min" },
                    { v: "120", l: "2 horas" },
                    { v: "150", l: "2 horas e 30 min" },
                    { v: "180", l: "3 horas" },
                    { v: "210", l: "3 horas e 30 min" },
                    { v: "240", l: "4 horas" },
                  ].map((d) => (
                    <SelectItem key={d.v} value={d.v} className="text-foreground">
                      {d.l}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  disabled={form.tipo === "MENSALISTA"}
                  className="bg-secondary border-border text-foreground disabled:opacity-50"
                />
              </div>
            </div>
            ...
            <div className="flex gap-2 pt-1">
              ...
              <Button
                className="flex-1"
                disabled={availableSlots.length === 0 || !inicioValido}
                onClick={() => setDialogOpen(false)}
              >
                Confirmar
              </Button>
