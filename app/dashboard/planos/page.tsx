...
const planos: { id: string; nome: string; valor: number; descricao: string; assinantes: number }[] = []

const mensalistas: { id: string; nome: string; plano: string; dia: string; horario: string; vencimento: string }[] = []
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {mensalistas.length > 0 ? `${mensalistas.length} mensalistas ativos` : "Nenhum mensalista ainda"}
          </p>
        </div>
      ...
      {tab === "planos" && (
        planos.length === 0 ? (
          <div className="bg-card border border-border rounded-xl p-10 text-center text-muted-foreground text-sm">
            Nenhum plano criado ainda. Clique em <strong>Novo Plano</strong> para começar.
          </div>
        ) : (
